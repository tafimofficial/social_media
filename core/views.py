...
def send_message_ajax(request):
    if request.method == 'POST':
        ...
        try:
            ...
            msg = Message.objects.create(sender=request.user, receiver=to_user, content=content)
...
def get_messages_ajax(request, username):
    ...
    # Get messages after a certain ID if needed, but for simple polling we'll just get last 20 or check new
    # Better: Client sends last_id. For now: Return all (inefficient but simple) or strict unread.
    # Let's do: Return messages NEWER than timestamp provided?
    # Simplest for this prompt: Return ALL conversation and let client render? No, too heavy.
    # Let's return messages where (sender=other, receiver=me, is_read=False) and mark them read.
